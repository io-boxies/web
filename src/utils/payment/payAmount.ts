...
import { type PayAmount, PAY_METHOD, PAID_INFO, type DefrayParam } from '..'

export const newPayAmount = (p: Partial<PayAmount>): PayAmount => ({
  tax: p.tax ?? 0,
  paidAmount: p.paidAmount ?? 0,
  paid: p.paid ?? PAID_INFO.NO,
  pureAmount: p.pureAmount ?? 0,
  amount: p.amount ?? 0,
  paymentConfirm: p.paymentConfirm ?? false,
  paymentMethod: p.paymentMethod ?? PAY_METHOD.CASH,
...
export const getPureAmount = (orderCnt: number, prodPrice: number) => orderCnt * prodPrice

export const getAmount = (a: { pureAmount: number; tax: number }) => a.pureAmount + a.tax

export function defrayAmount(target: PayAmount, d: Partial<DefrayParam>, isPending = false) {
  ...
  t.paymentMethod = d.payMethod ?? PAY_METHOD.CASH
...
export function refreshAmount(t: PayAmount) {
  if (t.amount === t.paidAmount) {
    t.paid = PAID_INFO.EXACT
  } else if (t.amount > t.paidAmount) {
    t.paid = PAID_INFO.CREDIT
  } else if (t.amount < t.paidAmount) {
    t.paid = PAID_INFO.OVERCOME
  }
}
